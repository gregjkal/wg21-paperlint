...
- `__main__.py` - CLI entry point. Argparse + paperstore-backed `convert_paper` invocation. Takes a paper id and a workspace dir; supports `--qa` mode with `--qa-json`, `--workers`, and `--timeout` flags for batch quality scoring. The pre-0.2 file-path interface (`tomd input.pdf`) is removed; sources must be staged via `python -m mailing` first.
