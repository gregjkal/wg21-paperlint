...
Since [P3981R0]: discussion at a LEWG telecon
lead to deciding to remove
try_append_range instead of
modifying it. See [P4022R0] (Remove
...
for the first three borrowed_subrange_t<R>
for the fourth-».
...
A previous revision of this paper made the argument that v.try_append_range(r)
should return a subrange instead of
an iterator (as pointed out by
PL-006). But we’ve decided it is best to remove the function entirely
for now, see [P4022R0] (Remove
try_append_range from
inplace_vector for
now). This paper no longer proposes any changes.
...
We propose to change the return types of three
algorithms in std::inplace_vector<T, N>:
try_emplace_back and both overloads
of try_push_back to return optional<T&>
instead of
T* And to
likewise change the return type of std::exception_ptr_cast
from E const*
to optional<E const&>.
...
[P4022R0] Barry Revzin, Jonathan Wakely, and Tomasz Kamiński.
2026-02-22. Remove try_append_range
from inplace_vector for now.
